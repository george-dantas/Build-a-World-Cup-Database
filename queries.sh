...
echo "$($PSQL "SELECT t.name FROM games g JOIN teams t ON g.winner_id = t.team_id WHERE year = 2018 and round = 'Final'")"
...
echo "$($PSQL "SELECT DISTINCT(t.name) FROM games g JOIN teams t ON g.winner_id = t.team_id OR g.opponent_id = t.team_id WHERE t.name ILIKE 'Co%'")"
